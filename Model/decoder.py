...
class CovCopyAttenGen:
    def __init__(self, options, is_training, embedding):
    ...
    def one_step_decoder(self, state_t_1, coverage_t_1, x, encoder_states, encoder_features, node_mask, v, w_c):
        '''
        ...
        for i in range(self.options['num_decoder_layer']):
            with tf.variable_scope('dec-layer-{}'.format(i)):
                cell = tf.contrib.rnn.LSTMCell(self.options['gen_hidden_size'],
                                                initializer=tf.random_uniform_initializer(-0.1, 0.1, seed=113))
                if self.options['dropout_rate'] > 0.0 and self.is_training:
                    cell = tf.contrib.rnn.DropoutWrapper(cell, output_keep_prob=1. - self.options['dropout_rate'])
                x, state_t_1[i] = cell(x, state_t_1[i])

        cell_output, state_t = x, state_t_1
        context_t, attn_dist, coverage_t = self.attention(state_t[-1], self.options['attention_vec_size'], encoder_states,
                                                             encoder_features, node_mask, v, w_c=w_c,
                                                             use_coverage = self.options['use_coverage'], coverage = coverage_t_1)
        # Calculate p_gen, Equation (8)
        p_gen = None
        if self.options['pointer_gen']:
        ...
        with variable_scope.variable_scope("AttnOutputProjection"):
            output_t = linear([cell_output] + [context_t], self.options['gen_hidden_size'], True)

        with tf.variable_scope('output_projection'):
            w = tf.get_variable('w', [self.options['gen_hidden_size'], self.options['tip_vocab_size']], dtype=tf.float32)
            b = tf.get_variable('b', [self.options['tip_vocab_size']], dtype=tf.float32)
            ...
            # if self.options['pointer_gen']:
            #     vocab_score_t = self.merge_prob_dist_for_one_step(vocab_score_t, attn_dist, p_gen, node_idxs, node_mask) # out_nodes_mask
    ...
    def train_mode(self, feas, encoder_dim, encoder_states, encoder_features, nodes_mask,
        ...
        state_t_1 = [init_state for _ in range(self.options['num_decoder_layer'])]
        ...
        with variable_scope.variable_scope("attention_decoder"):
            ...
            if self.options['use_coverage']:
                with variable_scope.variable_scope("coverage"):
                    w_c = variable_scope.get_variable("w_c", [self.options['attention_vec_size']])
            ...
            for i in range(self.options['max_tip_len'] - 1):
                ...
                x = linear([context_t_1], self.options['n_latent'], True)
                with tf.variable_scope('rating_feas'):
                    x = linear([word_t, x, feas], self.options['attention_vec_size'], True)

                (state_t, context_t, coverage_t, attn_dist_t, p_gen_t, output_t) = self.one_step_decoder(
                                state_t_1, coverage_t_1, x, encoder_states, encoder_features, nodes_mask, v, w_c)
        ...
        # calculating loss
        self._loss = None
        if mode_gen in ('ce_loss', 'rl_loss', 'greedy'):
            ...
            if self.options['use_coverage']:
                ...
                self._loss = self._loss + self.options['cov_loss_wt'] * self._coverage_loss

        # accuracy is calculated only under 'ce_loss' mode, where true answer is given
        return self._loss, sampled_words, attn_dists
