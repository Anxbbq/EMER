...
# DIR
TPS_DIR ='../../data/Yelp/yelp_2/'
META_TPS_DIR = '../../data/Yelp/'
SAVE_DIR = '../data/yelp'
CATEGORY = 'yelp'
...
tf.flags.DEFINE_string("meta_data", os.path.join(META_TPS_DIR, CATEGORY + '_meta.csv'), "meta_data")
...
def load_data(train_data, valid_data, test_data):
    ...
    with open(TPS_DIR+'user_item_num.txt') as f:
        user_num,item_num=np.fromfile(f, dtype=int, count=2, sep=" ")
...
def meta_data_process(meta_data):
    ...
    for line in f_meta:
        ...
        title.append(clean_str(line[2]).split(' '))
...
if __name__ == '__main__':
    ...
    pickle.dump(meta_infor, open(os.path.join(SAVE_DIR, CATEGORY + '.meta_infor'), 'wb'))
    ...
    pickle.dump(batches_train, open(os.path.join(SAVE_DIR, CATEGORY + '.train'), 'wb'))
    pickle.dump(batches_test, open(os.path.join(SAVE_DIR, CATEGORY + '.test'), 'wb'))
    ...
    pickle.dump(para_dict, open(os.path.join(SAVE_DIR, CATEGORY + '.para'), 'wb'))
